...
const getOrderOrderID = `-- name: GetOrderOrderID :many
SELECT
    ord.or_orderid,
    ord.or_totalamount,
    ord.or_orderstatus,
...
type GetOrderOrderIDRow struct {
	OrOrderid       uuid.UUID
...
func (q *Queries) GetOrderOrderID(ctx context.Context, orOrderid uuid.UUID) ([]GetOrderOrderIDRow, error) {
	rows, err := q.db.Query(ctx, getOrderOrderID, orOrderid)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetOrderOrderIDRow
	for rows.Next() {
		var i GetOrderOrderIDRow
		if err := rows.Scan(
			&i.OrOrderid,
			&i.OrTotalamount,
			&i.OrOrderstatus,
...
func (q *Queries) GetOrderProducts(ctx context.Context, orpOrderid uuid.UUID) ([]OrdersProduct, error) {
